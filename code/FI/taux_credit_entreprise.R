...
subtt = paste("Dernier point:",as.yearmon(df$obsTime[dim(df)[1]]))
subtt = paste(subtt, ", Fait le : " , time_now)
...
ggplot(data = df, aes(x = date, y = obsValue, color = REF_AREA, group = REF_AREA)) +
  geom_line(size=1.2) + 
  ggtitle("Taux des crédits nouveaux aux entreprises (%), BCE") + 
  ...
  scale_colour_discrete(breaks = breaks, labels = labels)

runtime_end = Sys.time()
run_time = as.numeric(difftime(runtime_end, runtime_start), units = "secs")


export_minio_graph(gg_taux_credit_entreprise,
                   perim = "FI", folder_name = "taux_credit_entreprise",

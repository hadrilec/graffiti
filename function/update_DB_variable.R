...
update_DB_variable <- function(){
  ...
  data_in_DB = aws.s3::get_bucket(Sys.getenv("AWS_BUCKET"),  region = "")
  ...
  aws.s3::s3write_using(DB_variable, FUN = saveRDS,
                bucket = Sys.getenv("AWS_BUCKET"), object = "dataviz/DB_variable/DB_variable",
                opts = list( "region" = ""))
...
dwn_minio_file = function(obj){
  ...
  if(obj %in% list_file){
    ...
    aws.s3::save_object(obj, 
                file = file_dwn,
                bucket = Sys.getenv("AWS_BUCKET"),  region = "")

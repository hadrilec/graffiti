...
add_style = function(gg, lang = "fr",
  ...
  if(lang == "en"){
    caption_text_start = "Made on"
    # caption_text_series = "Used series"
  }else{
    caption_text_start = "Fait le"
    # caption_text_series = "S\u00E9ries utilis\u00E9es"
  }

  if("DATE" %in% names(data)){
    last_update_date = data %>% pull(DATE) %>% max()
    subtt2 = sprintf("Derni\U00E8re date : %s", last_update_date)
    caption_text_added = sprintf("%s : %s, source : INSEE, %s",
                                 caption_text_start, lubridate::now(), subtt2
    )
  }else{
